...
// server holds the shared engine; every tool call goes through it, so MCP and
// CLI behaviour cannot diverge.
type server struct {
	e       *engine.Engine
	cfg     *config.Config
	version string
...
func (s *server) initializeResult(params json.RawMessage) any {
	...
	return map[string]any{
		...
		"serverInfo":      map[string]any{"name": "urlscan-lookup", "version": s.version},
